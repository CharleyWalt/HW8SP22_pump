...
class rankineController():
    ...
    def updateUnits(self, *args, SI=False):
        le_PHigh, le_PLow, le_TurbineInletCondition, lbl_PHigh, lbl_PLow, lbl_H1Units, lbl_H2Units, lbl_H3Units, lbl_H4Units, lbl_TurbineWorkUnits, lbl_PumpWorkUnits, lbl_HeatAddedUnits = args[1]
        self.Model.p_high = float(le_PHigh.text()) * UC.bar_to_psi
        self.Model.p_low = float(le_PLow.text()) * UC.bar_to_psi
        # self.Model.turbine_eff = float(le_TurbineInletCondition.text())
        self.calc_efficiency()
        self.updateView(self.Widgets)
